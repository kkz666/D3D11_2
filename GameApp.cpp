...
void GameApp::DrawScene()
{
	...
	if (m_CurrMode == ShowMode::WoodCrate) {
		for (int i = 0; i < 6; ++i)
		{
			m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pWoodCrate[i].GetAddressOf());
			m_pd3dImmediateContext->DrawIndexed(6, 6 * i, 0);
		}
	}
	...
	if (m_pd2dRenderTarget != nullptr)
	{
		...
		m_pd2dRenderTarget->DrawTextW(textStra, (UINT)wcslen(textStra), m_pTextFormat.Get(),
			D2D1_RECT_F{ 450.0f, 0.0f,6000.0f, 0.0f }, m_pColorBrush.Get());
...
bool GameApp::InitResource()
{
	...
	// 初始化木箱纹理
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"E:\\D3D11\\Texture\\flare.dds", nullptr, m_pWoodCrate[0].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"E:\\D3D11\\Texture\\flarealpha.dds", nullptr, m_pWoodCrate[1].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"E:\\D3D11\\Texture\\checkboard.dds", nullptr, m_pWoodCrate[2].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"E:\\D3D11\\Texture\\WireFence.dds", nullptr, m_pWoodCrate[3].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"E:\\D3D11\\Texture\\floor.dds", nullptr, m_pWoodCrate[4].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"E:\\D3D11\\Texture\\ice.dds", nullptr, m_pWoodCrate[5].GetAddressOf()));
	// 初始化火焰纹理
	WCHAR strFile[40];
	m_pFireAnims.resize(120);
	for (int i = 1; i <= 120; ++i)
	{
		wsprintf(strFile, L"F:\\D3D11_2\\Texture\\FireAnim\\Fire%03d.bmp", i);
	...
	m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pWoodCrate[0].GetAddressOf());
	m_pd3dImmediateContext->PSSetShaderResources(1, 1, m_pWoodCrate[1].GetAddressOf());
	m_pd3dImmediateContext->PSSetShader(m_pPixelShader3D.Get(), nullptr, 0);
